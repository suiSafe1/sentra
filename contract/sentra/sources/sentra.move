...
public entry fun complete_yield_withdrawal_with_redeemed_coin<CoinType, SCoin>(
    ...
    let (user_principal, penalty) = if (now >= unlock_time) {
        ...
        let actual_penalty = if (pen > total_redeemed_amount) { total_redeemed_amount } else { pen };
        let user_gets = if (available_principal > actual_penalty) { available_principal - actual_penalty } else { 0 };
